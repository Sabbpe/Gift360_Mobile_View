      ...
      <div className="absolute top-[35px] left-[125px] z-10 flex items-center gap-0.5 pointer-events-none">
        {["Apply SC", "Applied", "₹100 saved"].map((label, i) => (
          <span key={label} className="inline-flex items-center gap-0.5">
            {i > 0 && <span className="text-[7px] text-[#E11D48]/50 font-extrabold">→</span>}
            <span className="inline-flex items-center gap-[2px] rounded-full bg-white/80 backdrop-blur-sm px-1.5 py-[2px] shadow-[0_1px_4px_rgba(225,29,72,0.12)]">
              <img src={superCoinImg} alt="" className="w-[7px] h-[7px] object-contain" />
              <span className="text-[7px] font-extrabold text-[#E11D48] leading-none whitespace-nowrap">{label}</span>
            </span>
          </span>
        ))}
      </div>
