...
const DEMO_STORAGE_KEY = "sc_nudge_seen_bata";
...
export default function BataSuperCoinNudge({ onExplore }: Props) {
  ...
  const handleCardTap = useCallback(() => {
    ...
    setLocation(`/brand/${BATA_BRAND_ID}`);
  ...
  return (
    <div className="relative w-full h-[185px] overflow-hidden rounded-xl" onClick={handleCardTap}>
      ...
      {/* Bata backdrop */}
      <img src={bataBg} alt="" className="absolute inset-0 h-full w-full object-cover object-center pointer-events-none" />
      ...
      {/* SC guide text — beside "Bata shoes" on backdrop */}
      <div className="absolute top-[34px] left-[165px] z-10 flex items-center gap-1 pointer-events-none">
        {["Apply SC", "SuperCoins Applied", "₹100 saved"].map((label, i) => (
          <span key={label} className="inline-flex items-center gap-1">
            {i > 0 && <span className="text-[9px] text-[#E11D48]/50 font-extrabold">→</span>}
            <span className="inline-flex items-center gap-[3px] rounded-full bg-white/80 backdrop-blur-sm px-2 py-[3px] shadow-[0_1px_4px_rgba(225,29,72,0.12)]">
              <img src={superCoinImg} alt="" className="w-2 h-2 object-contain" />
              <span className="text-[8px] font-extrabold text-[#E11D48] leading-none whitespace-nowrap">{label}</span>
            </span>
          </span>
        ))}
      </div>
      ...
      <div className="absolute left-4 bottom-2 z-10">
        <div className="relative w-[140px] h-[120px] rounded-xl bg-white/95 backdrop-blur-sm border border-red-100 shadow-[0_6px_20px_rgba(225,29,72,0.18)] overflow-hidden flex flex-col px-2.5 pt-2.5 pb-2 cursor-pointer active:scale-[0.98] transition-transform">
          ...
          <div className="flex items-center gap-1 mb-1">
            <div className="w-5 h-5 rounded-full bg-[#E11D48] flex items-center justify-center"><span className="text-[7px] font-extrabold text-white leading-none italic">Bata</span></div>
            <span className="text-[9px] font-bold text-gray-900">Bata</span>
          </div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center relative">
            ...
            <div className="h-[22px] flex items-center justify-center">
              <span key={toggleOn ? "on" : "off"} className="text-[17px] font-extrabold text-[#E11D48]" style={{ animation: toggleOn ? "price-in 0.35s cubic-bezier(0.34,1.56,0.64,1)" : undefined }}>
          ...
          <div className="flex items-center justify-between mt-auto pt-1 border-t border-gray-100">
            <span className="text-[7px] font-semibold text-[#E11D48] leading-tight">Toggle ON for demo</span>
            <div className="relative" onClick={(e) => e.stopPropagation()}>
              <button type="button" onClick={handleUserToggle} className="relative w-[32px] h-[18px] rounded-full transition-colors duration-300 focus:outline-none" style={{ background: toggleOn ? "linear-gradient(135deg,#E11D48,#BE123C)" : "#D1D5DB" }} aria-label="Toggle SuperCoins">
                ...
                {ghostVisible && (
                  <div className="absolute -top-5 left-1/2 -translate-x-1/2 pointer-events-none" style={{ animation: "ghost-down 0.4s ease-out forwards" }}>
                    ...
                    {demoPhase === "toggling" && <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-6 h-6 rounded-full border border-red-400/40" style={{ animation: "ripple 0.4s ease-out forwards" }} />}
                  </div>
                )}
              </button>
            </div>
